...
const setCVs = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(
      fetchLastCvCurrentUser.pending.toString(),
      (state: initialStateType) => {
        state.cvInfo.status = "loading";
      }
    );
    builder.addCase(
      fetchLastCvCurrentUser.fulfilled.toString(),
      (state: initialStateType, action: PayloadAction<ActionPayloadType>) => {
        if (action.payload) {
    ...
    builder.addCase(
      fetchLastCvCurrentUser.rejected.toString(),
      (state: initialStateType) => {
