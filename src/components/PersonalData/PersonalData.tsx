...
const PersonalData: React.FC<PersonalDataProps> = (props) => {
  ...
  const createResume = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    isValidEmail(props.userInfo.email)
      ? setError(null)
        ...
        <div>
          ...
          {props.userInfo.imageUrl ? (
            <div className={styles.withFoto}>
              <img
                ...
                src={`http://localhost:4434${props.userInfo.imageUrl}`}
                alt="Uploaded"
              />
              <Button
                sx={{
                ...
                onClick={props.onClickRemoveImage}
                variant="contained"
                color="error"
              >
            ...
            <Button
              ...
              size="medium"
            >
              <Avatar alt="userAvatar" src="" sx={{ width: 80, height: 80 }} />
              <p>Load foto</p>
            </Button>
          )}
          ...
          <section className={styles.histories}>
            ...
            <Button
              sx={buttonStyle}
              fullWidth
              onClick={props.addMoreWorkData}
              variant="outlined"
            >
              {props.childrenWorkHistoryArray.length > 0
                ? "Add More Work history"
                : "Add Work history"}
            </Button>
            <>
              {props.childrenEducationHistoryArray.map(
                (el: EducationHistoryType) => (
                  <EducationHistory
                    id={el.id}
                    key={el.id}
                    deleteEducationHistoryElement={
                      props.deleteEducationHistoryElement
                    }
                    getUserInfoData={props.getUserInfoData}
                  />
                )
            ...
            <Button
              sx={buttonStyle}
              fullWidth
              onClick={props.addEducationChildren}
              variant="outlined"
            >
              {props.childrenEducationHistoryArray.length > 0
                ? "Add more studies"
                : "Add Education history"}
            </Button>
            <>
              {props.childrenLanguageHistoryArray.map(
                (el: LanguageHistoryType) => (
                  <Languages
                    id={el.id}
                    key={el.id}
                    deleteLanguageHistoryElement={
                      props.deleteLanguageHistoryElement
                    }
                    getUserInfoData={props.getUserInfoData}
                  />
                )
            ...
            <Button
              sx={buttonStyle}
              fullWidth
              onClick={props.addLanguageChildren}
              variant="outlined"
            >
              {props.childrenLanguageHistoryArray.length > 0
                ? "Add more languages"
                : "Add language"}
            </Button>
            <>
            ...
            <Button
              sx={buttonStyle}
              fullWidth
              onClick={props.addSkillsChildren}
              variant="outlined"
            >
              {props.childrenSkillsHistoryArray.length > 0
                ? "Add more Skills"
                : "Add Skill"}
            </Button>
          </section>
